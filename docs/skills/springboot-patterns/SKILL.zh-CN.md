...
面向可扩展、生产级服务的 Spring Boot 架构和 API 模式。

## 何时激活

- 使用 Spring MVC 或 WebFlux 构建 REST API 时
- 构建 controller → service → repository 分层结构时
- 配置 Spring Data JPA、缓存或异步处理时
- 添加验证、异常处理或分页时
- 为开发/预发布/生产环境设置 profiles 时
- 使用 Spring Events 或 Kafka 实现事件驱动模式时
...
当 `ForwardedHeaderFilter` 正确配置后，`request.getRemoteAddr()` 将自动从转发头中返回正确的客户端 IP。如果没有此配置，请直接使用 `request.getRemoteAddr()`——它返回直接连接的 IP，这是唯一可信的值。
...
使用 Spring 的 `@Scheduled` 或集成消息队列（如 Kafka、SQS、RabbitMQ）。保持处理器幂等且可观测。
...
- 在适当位置通过 `@NonNull` 和 `Optional` 强制空安全

**请记住**：保持控制器精简、服务专注、仓储简单、错误集中处理。以可维护性和可测试性为优化目标。
